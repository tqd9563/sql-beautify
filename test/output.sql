with total_cnt as (
    ...
    select  media_src,
        sum(
            case
                when acc_create_os = 'android' then 1
                else 0
            end
        ) as android_cnt,
        sum(
            case
                when acc_create_os = 'ios' then 1
                else 0
            end
        ) as ios_cnt,
        sum(
            case
                when acc_create_os = 'win' then 1
                else 0
            end
        ) as win_cnt
    from total_cnt
...
select  media_src,
    ...
    when android_cnt > 0
    and ios_cnt = 0
    and win_cnt = 0 then 'android'
    when ios_cnt > 0
    and android_cnt = 0
    and win_cnt = 0 then 'ios'
    when win_cnt > 0
    and android_cnt = 0
    and ios_cnt = 0 then 'win'
    end as exclusive_os
from media_cnt
where (
    android_cnt > 0
